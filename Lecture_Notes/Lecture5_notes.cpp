...
//     scattered in different memory locations
...
// Without the * operator, the identifier x refers to the pointer itself, not the
// value it points to
...
// values you might want to forbid changing:
...
int * const ptr; // declares a constant pointer to changeable integer data
                 ...
                 // the pointer may not be changed to point to a different
                 // constant integer

const int * const ptr; // forbids changing either the address ptr contains or
